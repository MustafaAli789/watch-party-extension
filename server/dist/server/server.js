...
io.on(constants_1.SocketEvents.CONNECTION, (socket) => {
    ...
    socket.on(constants_1.SocketEvents.JOIN, (joinRoomData, callback) => {
        ...
        if (joinRoomData.action == constants_1.RoomAction.CREATE) {
            ...
            const { error } = util_1.addUserToRoom(socket.id, joinRoomData.userName, joinRoomData.roomId, false);
            if (error) {
                return callback(error);
            }
            socket.join(joinRoomData.roomId);
            ...
            socket.broadcast.to(joinRoomData.roomId).emit(constants_1.SocketEvents.USER_CONNECTED, { message: `User with name: ${joinRoomData.userName} has joined the room` });
        }
        else {
