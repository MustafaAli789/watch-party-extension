...
const socketEvents_1 = __webpack_require__(/*! ../socketEvents */ "../socketEvents.ts");
...
const establishSocketConnection = ({ roomName, userName }, sendResponse) => {
    ...
    let roomId = guid();
    socket.emit(socketEvents_1.default.JOIN, { roomName: roomName, userName: userName, roomId: roomId, action: "CREATE" }, (err) => {
        // shouldnt happen theoretically
        alert(err);
    });
    socket.on(socketEvents_1.default.CREATED_ROOM, ({ payload }) => {
        sendResponse({
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.message === constants_1.Messages.TOFG_VIDEO_ON_SCREEN) {
        ...
        socket.emit(socketEvents_1.default.FORCE_DISCONNECT);
        sendResponse({
            status: constants_1.Messages.SUCCESS
        });
    }
