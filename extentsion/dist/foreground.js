...
const createSocketConnection = (roomData, sendResponse) => {
    ...
    socket.on(constants_2.SocketEvents.USER_CHANGE, (data) => {
        if (data.changeEvent === constants_2.UserChange.JOIN) {
            addNotif({ headerMsg: 'User Joined', type: 'NOTIF', bodyMsg: `User ${data.changedUser.userName} joined room.` });
        }
        else if (data.changeEvent === constants_2.UserChange.DISCONNECT) {
