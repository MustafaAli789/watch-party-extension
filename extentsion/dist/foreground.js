...
const createSocketConnection = (roomData, sendResponse) => {
    ...
    socket.on(constants_2.SocketEvents.CONNECTED_TO_ROOM, (data) => {
    ...
    socket.on(constants_2.SocketEvents.USER_CHANGE, (data) => {
        ...
        if (data.changeEvent === constants_2.UserChange.JOIN && data.admin.userId === socket.id) {
            if (vidElem.readyState <= 2) {
            }
            socket.emit(constants_2.SocketEvents.VIDEO_EVENT, {
                videoEvent: constants_2.VideoEvent.JOIN,
                videoData: retrieveVideoData(),
                userIdToSendTo: data.changedUser.userId,
                triggeringUserId: socket.id,
                error: vidElem.readyState <= 2 ? 'Admins video is buffering. Please rejoin.' : null
            });
        }
        //NOTIFICATION HERE
    });
